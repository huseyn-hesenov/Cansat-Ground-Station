...
view.show()
view.setWindowTitle('CanSat Azerbaijan')
...
# Title at top
text = """
Fenix komandasının Cansat yer üstü stansiyası
"""
Layout.addLabel(text, col=1, colspan=21)
Layout.nextRow()

# Put vertical label on left side
Layout.addLabel('FENİX',
                angle=-90, rowspan=3)
...
# buttons style
style = "background-color:rgb(255, 255, 0);color:rgb(0,0,0);font-size:16px;"

lb = Layout.addLayout(colspan=21)
proxy = QtGui.QGraphicsProxyWidget()
save_button = QtGui.QPushButton('Başlat və yadda saxla')
...
end_save_button = QtGui.QPushButton('Dayandır və yadda saxla')
...
p1 = l11.addPlot(title="Hündürlük (m)")
...
# Speed graph
p2 = l11.addPlot(title="Sürət (m/s)")
...
# Acceleration graph
acc_graph = l12.addPlot(title="Təcil (m/s²)")
# adding legend
acc_graph.addLegend()
...
# Gyro graph
gyro_graph = l12.addPlot(title="Giroskop")
gyro_graph.hideAxis('bottom')
# adding legend
gyro_graph.addLegend()
pitch_plot = gyro_graph.plot(pen=(102, 252, 241), name="meydan")
roll_plot = gyro_graph.plot(pen=(29, 185, 84), name="diyircəkli")
yaw_plot = gyro_graph.plot(pen=(203, 45, 111), name="trübülans")
...
# Pressure Graph
pressure_graph = l12.addPlot(title="Barometrik təzyiq")
...
# Temperature graph
graf_temp = l12.addPlot(title="Temperatur (ºc)")
...
# Time graph
time_graph = l2.addPlot(title="Çalışma müddəti(dəq)")
...
# Battery graph
battery_graph = l2.addPlot(title="Batareya vəziyyəti")
...
freeFall_graph = l2.addPlot(title="Sərbəst düşmə")
